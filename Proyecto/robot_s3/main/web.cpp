/*
 * Robot de Ring — ESP32-S3 + L298N + ESP32-CAM por UART
 * HTML del controlador embebido — acceder en http://192.168.4.1
 *
 * Ordenes desde la web: MANUAL / FIND (buscar y destruir) / PATROL / RETREAT.
 * Los datos de vision (borde + identificador) llegan por UART1 desde la
 * ESP32-CAM (ver uart_link.h); la maquina de estados vive en control.cpp.
 *
 * LED RGB integrado WS2812 (GPIO 48) via espressif/led_strip
 *
 * Pines L298N:                       Pines UART1 (CAM):
 *   ENA → GPIO 4   ENB → GPIO 7        TX → GPIO 17 (reservado)
 *   IN1 → GPIO 5   IN3 → GPIO 15       RX → GPIO 18 (datos CAM)
...
<title>Robot Ring</title>
<style>
  :root{--bg:#0a0a0f;--panel:#13131a;--acc:#00e5ff;--btn:#1e1e2e;--btnH:#2a2a3e;--btnA:#00e5ff22;--txt:#e8e8f0;--muted:#6b6b80;--green:#00ff88;--red:#ff4466;--mag:#ff66ff;--yel:#ffe066;}
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .si{flex:1;min-width:60px;background:var(--panel);border:1px solid #2a2a3a;border-radius:8px;padding:.5rem;text-align:center;}
  ...
  .mode-btn{flex:1;background:var(--btn);border:1px solid #2a2a3a;border-radius:10px;color:var(--txt);font-size:.62rem;letter-spacing:.05em;padding:.55rem .2rem;cursor:pointer;transition:background .07s,border-color .07s;text-transform:uppercase;}
  .mode-btn:hover{background:var(--btnH);border-color:var(--acc);}
  .mode-btn.active{background:#00ff8822;border-color:var(--green);color:var(--green);}
...
<div class="app">
  <div class="header">
    <h1>&#9632; ROBOT RING</h1>
    <p>ESP32-S3 + ESP32-CAM (UART) &mdash; ICC4200</p>
  </div>
  <div class="sbar">
    <div class="si"><div class="sl">Conexion</div><div class="sv off" id="cs">OFFLINE</div></div>
    <div class="si"><div class="sl">Modo</div><div class="sv off" id="modeS">MANUAL</div></div>
    <div class="si"><div class="sl">Cam</div><div class="sv off" id="linkS">SIN CAM</div></div>
    <div class="si"><div class="sl">Borde</div><div class="sv dir" id="edgeS">-</div></div>
    <div class="si"><div class="sl">Ident</div><div class="sv dir" id="identS">-</div></div>
  </div>
  <div class="mrow">
  ...
  <div class="spd">
    <div class="sph"><span>Velocidad manual (PWM)</span><strong id="sl2">70%</strong></div>
...
async function setMode(m){
  try{
    ...
    addLog('Modo -> '+m.toUpperCase(),'ok');
  }catch(e){
    addLog('ERR modo '+m+': '+e.message,'err');
  }
}
...
addLog('Conectado al ESP32-S3. Botones, WASD o modos autonomos.','info');
setInterval(async()=>{
  try{
    ...
    mS.textContent=(j.mode==='manual'?'MANUAL':(j.mode+'/'+j.sub)).toUpperCase();
    mS.className='sv '+(j.mode==='manual'?'dir':'on');
    document.getElementById('edgeS').textContent=j.dist.join(',');
    document.getElementById('identS').textContent=(j.ident?'SI ':'no ')+j.conf+'%';
    const lS=document.getElementById('linkS');
    lS.textContent=j.link?'OK':'SIN CAM';
    lS.className='sv '+(j.link?'on':'off');
    document.querySelectorAll('.mode-btn').forEach(b=>b.classList.toggle('active',b.dataset.mode===j.mode));
    const cs=document.getElementById('cs');cs.textContent='ONLINE';cs.className='sv on';
  }catch(e){
    const cs=document.getElementById('cs');cs.textContent='OFFLINE';cs.className='sv off';
  }
},500);
</script>
</body>
...
// Un comando manual saca al robot de cualquier modo autonomo
static inline void ensureManual() {
  if (control_get_mode() != MODE_MANUAL) control_set_mode(MODE_MANUAL);
}
...
// ── Handlers de modo y estado ──────────────────────────────
static esp_err_t hMode(httpd_req_t *req) {
  char buf[32] = {0};
  char val[12] = {0};
  httpd_req_get_url_query_str(req, buf, sizeof(buf) - 1);
  httpd_query_key_value(buf, "set", val, sizeof(val) - 1);
  ...
  if      (strcmp(val, "find")    == 0) m = MODE_FIND;
    ...
    httpd_resp_send(req, "bad mode", 8);
    return ESP_OK;
  }
  ...
  httpd_resp_set_type(req, "text/plain");
  char out[24];
  int len = snprintf(out, sizeof(out), "mode:%s", val);
...
static esp_err_t hStatus(httpd_req_t *req) {
  char json[192];
  int len = control_status_json(json, sizeof(json));
  cors(req);
...
extern "C" void app_main() {
  ...
  httpd_uri_t uri_mode     = {.uri="/mode",     .method=HTTP_GET,     .handler=hMode,     .user_ctx=NULL};
  httpd_uri_t uri_status   = {.uri="/status",   .method=HTTP_GET,     .handler=hStatus,   .user_ctx=NULL};
