...
static TaskHandle_t  g_auto_task  = NULL;
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .auto-btn{width:100%;background:var(--btn);border:1px solid #2a2a3a;border-radius:10px;color:var(--txt);font-size:.78rem;letter-spacing:.07em;padding:.55rem;cursor:pointer;transition:background .07s,border-color .07s;text-transform:uppercase;}
  .auto-btn.active{background:#00ff8822;border-color:var(--green);color:var(--green);}
...
<div class="app">
  ...
  <button class="auto-btn" id="autoBtn" onclick="toggleAuto()">&#9654; MODO AUTO</button>
  <div class="dpad">
    <div class="de"></div>
...
let autoOn=false,pollIv=null;
function startDetectionPoll(){
  if(pollIv)return;
  pollIv=setInterval(async()=>{
    try{
      const r=await fetch('/cam-ip',{cache:'no-store'});
      if(!r.ok)return;
      const ip=(await r.text()).trim();
      if(!ip||ip==='none')return;
      const dr=await fetch('http://'+ip+':81/detection',{cache:'no-store'});
      if(!dr.ok)return;
      const j=await dr.json();
      document.getElementById('edgeS').textContent=(j.edge||'-').toUpperCase();
    }catch(e){}
  },500);
}
async function toggleAuto(){
  ...
  try{
    await fetch('/auto?on='+(autoOn?'1':'0'),{cache:'no-store'});
    if(autoOn){
      btn.textContent='⏹ DETENER AUTO';
      btn.classList.add('active');
      document.getElementById('modeS').textContent='AUTO';
      document.getElementById('modeS').className='sv on';
      addLog('Modo AUTO activado','ok');
      startDetectionPoll();
    }else{
      btn.textContent='▶ MODO AUTO';
      btn.classList.remove('active');
      document.getElementById('modeS').textContent='MANUAL';
      document.getElementById('modeS').className='sv off';
      addLog('Modo AUTO desactivado','info');
    }
  }catch(e){addLog('Error toggling AUTO: '+e.message,'err');autoOn=!autoOn;}
}
</script>
</body>
</html>
...
static esp_err_t auto_http_event(esp_http_client_event_t *evt) {
    if (evt->event_id == HTTP_EVENT_ON_DATA && evt->data_len > 0) {
        ...
        if (s_auto_resp_len + copy >= (int)sizeof(s_auto_resp) - 1)
            copy = sizeof(s_auto_resp) - 1 - s_auto_resp_len;
        memcpy(s_auto_resp + s_auto_resp_len, evt->data, copy);
        s_auto_resp_len += copy;
        s_auto_resp[s_auto_resp_len] = '\0';
    }
    return ESP_OK;
}
...
static void autoTask(void *) {
    while (true) {
        ...
        char url[64];
        snprintf(url, sizeof(url), "http://%s:81/detection", g_cam_ip);
...
// ── Handlers AUTO ──────────────────────────────────────────
static esp_err_t hCamRegister(httpd_req_t *req) {
    char buf[64] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf) - 1);
    char ip[20] = {0};
    if (httpd_query_key_value(buf, "ip", ip, sizeof(ip) - 1) == ESP_OK) {
        strncpy(g_cam_ip, ip, sizeof(g_cam_ip) - 1);
        ESP_LOGI(TAG, "CAM registrada ip=%s", g_cam_ip);
    }
    cors(req);
...
static esp_err_t hCamIp(httpd_req_t *req) {
    cors(req);
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_send(req, g_cam_ip[0] ? g_cam_ip : "none", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

static esp_err_t hAuto(httpd_req_t *req) {
    char buf[16] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf) - 1);
    char val[4] = {0};
    httpd_query_key_value(buf, "on", val, sizeof(val) - 1);
    bool on = (val[0] == '1');
    g_auto_mode = on;
    if (on) {
        if (!g_auto_task)
            xTaskCreate(autoTask, "autoTask", 8192, NULL, 5, (TaskHandle_t*)&g_auto_task);
        ESP_LOGI(TAG, "[AUTO] activado");
    } else {
        motorStop(); setLED(0, 0, 40);
        ESP_LOGI(TAG, "[AUTO] desactivado");
    }
    cors(req);
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_send(req, on ? "auto:on" : "auto:off", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
extern "C" void app_main() {
  ...
  httpd_uri_t uri_cam_reg  = {.uri="/cam-register", .method=HTTP_GET, .handler=hCamRegister, .user_ctx=NULL};
  httpd_uri_t uri_cam_ip   = {.uri="/cam-ip",       .method=HTTP_GET, .handler=hCamIp,       .user_ctx=NULL};
  httpd_uri_t uri_auto     = {.uri="/auto",          .method=HTTP_GET, .handler=hAuto,        .user_ctx=NULL};

  httpd_register_uri_handler(server, &uri_cam_reg);
  httpd_register_uri_handler(server, &uri_cam_ip);
